...
// This file is the given code that contains the functions for the 
// LCD display. LCD_GPIOInit() functions is implemented following the comment.
// PrintFloat and required header file are revised.
...
static void LCD_GPIOInit(void){
    ...
    wait++;                                     // wait for port activation
    wait++;                                     // wait for port activation
    // make PN4, PN5 outputs
    GPIO_PORTN_DIR_R |= (0x1 << 4);
    GPIO_PORTN_DIR_R |= (0x1 << 5);
    // disable alternate functions
    GPIO_PORTN_AFSEL_R &= ~(0x1 << 4);
    GPIO_PORTN_AFSEL_R &= ~(0x1 << 5);
    // enable digital I/O on PN4, PN5
    GPIO_PORTN_DEN_R |= (0x1 << 4);
    GPIO_PORTN_DEN_R |= (0x1 << 5);
    ...
    GPIO_PORTP_DIR_R |= (0x1 << 1);
    GPIO_PORTP_DIR_R |= (0x1 << 3);
    GPIO_PORTP_DIR_R |= (0x1 << 4);
    // disable alternate functions
    GPIO_PORTP_AFSEL_R &= ~(0x1);
    GPIO_PORTP_AFSEL_R &= ~(0x1 << 1);
    GPIO_PORTP_AFSEL_R &= ~(0x1 << 3);
    GPIO_PORTP_AFSEL_R &= ~(0x1 << 4);    
    // enable digital I/O on PP0-1, PP3-4
    GPIO_PORTP_DEN_R |= (0x1);
    GPIO_PORTP_DEN_R |= (0x1 << 1);
    GPIO_PORTP_DEN_R |= (0x1 << 3);
    GPIO_PORTP_DEN_R |= (0x1 << 4);
    ...
    // make PQ0, PQ2-3 outputs
     GPIO_PORTQ_DIR_R |= (0x1);
     GPIO_PORTQ_DIR_R |= (0x1 << 2);
     GPIO_PORTQ_DIR_R |= (0x1 << 3);
     // disable alternate functions
     GPIO_PORTQ_AFSEL_R &= ~(0x1);
     GPIO_PORTQ_AFSEL_R &= ~(0x1 << 2);
     GPIO_PORTQ_AFSEL_R &= ~(0x1 << 3);
     // enable digital I/O on PQ0, PQ2-3
     GPIO_PORTQ_DEN_R |= (0x1);
     GPIO_PORTQ_DEN_R |= (0x1 << 2);
     GPIO_PORTQ_DEN_R |= (0x1 << 3);
}
